...
/* Wrapper that records writes when floor1_encode is active for dump */
static int g_floor1_recording = 0;

static void dump_oggpack_write(oggpack_buffer *opb, unsigned long value, int bits){
  oggpack_write(opb, value, bits);
  if(g_floor1_recording && g_floor_bits_count < DUMP_MAX_BITS){
    g_floor_bits[g_floor_bits_count].value = (int)value;
    g_floor_bits[g_floor_bits_count].bits  = bits;
    g_floor_bits_count++;
  }
}
#define DUMP_WRITE(opb,v,b) dump_oggpack_write(opb,(unsigned long)(v),(b))
...
static int fit_line(lsfit_acc *a,int fits,int *y0,int *y1,
  ...
  if(getenv("LV_DEBUG_FITLINE") && fits>=20){
    fprintf(stderr,"LV_FITLINE fits=%d x0=%d x1=%d: xb=%.1f yb=%.1f x2b=%.1f xyb=%.1f bn=%.1f\n", fits, x0, x1, xb, yb, x2b, xyb, bn);
  }
...
int floor1_encode(oggpack_buffer *opb,vorbis_block *vb,
  ...
  long _bits_before = oggpack_bits(opb);
  if(dump_now && !dump_done){
    g_floor1_recording = 1;
    g_floor_bits_count = 0;
  }
  vorbis_info_floor1 *info=look->vi;
  long posts=look->posts;
  ...
  if(post){
    ...
    DUMP_WRITE(opb,out[0],ov_ilog(look->quant_q-1));
    DUMP_WRITE(opb,out[1],ov_ilog(look->quant_q-1));
...

    {
      ...
      if(g_floor1_recording){
        g_floor1_recording = 0;
        /* Record total bits emitted as a single entry for convenience */
        if(g_floor_bits_count < DUMP_MAX_BITS){
          g_floor_bits[g_floor_bits_count].value = (int)(oggpack_bits(opb) - _bits_before);
          g_floor_bits[g_floor_bits_count].bits  = -1; /* sentinel: total bits */
          g_floor_bits_count++;
        }
        dump_done = 1;
      }
