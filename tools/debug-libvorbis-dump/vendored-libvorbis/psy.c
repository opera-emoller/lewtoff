...
/* DEBUG globals for offset_and_mix tracing */
static int oam_call_count = 0;
...
void _vp_offset_and_mix(vorbis_look_psy *p,
  ...
  if(oam_call_count < 6 && offset_select==1) {
    fprintf(stderr,"OAM_DEBUG call=%d n=%d toneatt=%.6f noiseoffset[1][0]=%.6f noise[0]=%.6f tone[0]=%.6f noisemaxsupp=%.6f\n",
      oam_call_count, n, toneatt, p->noiseoffset[1][0], noise[0], tone[0], p->vi->noisemaxsupp);
    fprintf(stderr,"OAM_DEBUG: val_pre_clamp=%.6f val_post_clamp=%.6f tone_val=%.6f expected_logmask=%.6f\n",
      noise[0]+p->noiseoffset[1][0],
      (noise[0]+p->noiseoffset[1][0])>p->vi->noisemaxsupp ? p->vi->noisemaxsupp : (noise[0]+p->noiseoffset[1][0]),
      tone[0]+toneatt,
      max((noise[0]+p->noiseoffset[1][0])>p->vi->noisemaxsupp ? p->vi->noisemaxsupp : (noise[0]+p->noiseoffset[1][0]),
          tone[0]+toneatt));
    oam_call_count++;
  }
...
// Temporary debug: print bark values
void debug_bark_values(vorbis_look_psy *p) {
  for (int i = 10; i <= 30; i++) {
    long b = p->bark[i];
    long lo = b >> 16;
    long hi = b & 0xffff;
    fprintf(stderr, "LV_BARK[%d]: lo=%ld hi=%ld\n", i, lo, hi);
  }
}
