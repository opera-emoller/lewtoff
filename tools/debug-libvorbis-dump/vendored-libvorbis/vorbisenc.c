...
static void vorbis_encode_noisebias_setup(vorbis_info *vi,double s,int block,
  ...
  fprintf(stderr,"NOISEBIAS_SETUP_DEBUG: block=%d s=%.6f is=%d ds=%.6f in[is].data[1][0]=%d in[is+1].data[1][0]=%d noiseoff[1][0]=%.6f userbias=%.6f\n",
          block, s, is, ds, in[is].data[1][0], in[is+1].data[1][0], p->noiseoff[1][0], userbias);
