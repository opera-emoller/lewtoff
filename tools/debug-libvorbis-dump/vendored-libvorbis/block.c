...
static void _preextrapolate_helper(vorbis_dsp_state *v){
  ...
  if(v->pcm_current-v->centerW>order*2){ /* safety */
    for(i=0;i<v->vi->channels;i++){
      ...
      /* DEBUG: print LPC coefficients */
      fprintf(stderr,"LV_LPC_COEFFS: [");
      for(int dbgk=0;dbgk<order;dbgk++) fprintf(stderr,"%.10e%s",lpc[dbgk],dbgk<order-1?",":"");
      fprintf(stderr,"]\n");
      /* DEBUG: print prime values */
      fprintf(stderr,"LV_LPC_PRIME: [");
      for(int dbgk=0;dbgk<order;dbgk++) fprintf(stderr,"%.10e%s",work[v->pcm_current-v->centerW-order+dbgk],dbgk<order-1?",":"");
      fprintf(stderr,"]\n");
      ...
      /* DEBUG: print first few predicted values */
      fprintf(stderr,"LV_LPC_PREDICTED_0to7: [");
      for(int dbgk=0;dbgk<8;dbgk++) fprintf(stderr,"%.10e%s",work[v->pcm_current-v->centerW+dbgk],dbgk<7?",":"");
      fprintf(stderr,"]\n");
...
int vorbis_analysis_wrote(vorbis_dsp_state *v, int vals){
  ...
  if(vals<=0){
    ...
    if(!v->preextrapolate && v->pcm_current-v->centerW>ci->blocksizes[1]){
      fprintf(stderr,"LV_PREEXTRAPOLATE: pcm_current=%ld centerW=%ld n_lpc=%ld\n",
              (long)v->pcm_current, (long)v->centerW, (long)(v->pcm_current-v->centerW));
      _preextrapolate_helper(v);
    }
